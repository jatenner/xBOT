...
interface AIDecision {
  ...
  decisionType: 'posting_frequency' | 'timing' | 'content_type' | 'strategy' | 'competitive';
  recommendation: any;
  confidence: number;
  reasoning: string;
...
export class UnifiedDataManager {
  ...
  /**
   * 📝 STORE POST DATA (Single source of truth)
   */
  public async storePost(postData: UnifiedPost): Promise<void> {
    console.log(`📝 UNIFIED_DATA: Storing post ${postData.postId} with comprehensive data`);
