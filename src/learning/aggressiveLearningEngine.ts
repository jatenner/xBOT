...
export class AggressiveLearningEngine {
  ...
  private getAggressiveStrategy(hour: number, postsToday: number): PostingStrategy {
    ...
    // Convert UTC to Pacific Time (Railway runs in UTC, need Pacific Time)
    const now = new Date();
    const pacificTime = new Date(now.toLocaleString("en-US", {timeZone: "America/Los_Angeles"}));
    const localHour = pacificTime.getHours();
    const isActiveHour = localHour >= 5 && localHour <= 23;
    
    console.log(`🕐 TIME_CHECK: UTC=${hour}, Pacific=${localHour}, Active=${isActiveHour}`);
    ...
    // Randomize content type for data diversity - BALANCED WITH MORE THREADS  
    const types: Array<'simple' | 'thread' | 'reply'> = ['thread', 'simple', 'thread', 'simple', 'thread']; // Weighted 60% threads
