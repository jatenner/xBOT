...
/**
 * 🐛 DEBUG: Seed and run scheduler (in-process, uses same pool)
 * POST /debug/seed-and-run
 * Body: { count: 10 }
 * Header: Authorization: Bearer <DEBUG_TOKEN>
 */
app.post('/debug/seed-and-run', async (req, res) => {
  try {
    // Check auth token
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ') || authHeader.substring(7) !== debugToken) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    
    const count = parseInt(req.body.count || '10', 10);
    
    console.log(`[DEBUG_SEED] Seeding ${count} candidates and triggering scheduler...`);
    
    // Step 1: Seed candidates (reuse seed logic)
    ...
    for (let i = 0; i < count; i++) {
      const tweetId = String(baseTweetId + BigInt(Date.now()) + BigInt(i));
      const evaluationId = uuidv4();
      
      // Create candidate_evaluation entry
      const { error: evalError } = await supabase
        ...
        .insert({
          ...
          candidate_posted_at: new Date(now.getTime() - i * 60 * 1000).toISOString(),
          ...
          status: 'evaluated',
          source_type: 'debug_seed',
          source_feed_name: 'debug_test',
          created_at: now.toISOString(),
        });
      
      if (evalError) {
        console.error(`[DEBUG_SEED] Failed to create evaluation for ${tweetId}: ${evalError.message}`);
        continue;
      }
      ...
      if (queueError) {
        console.error(`[DEBUG_SEED] Failed to queue ${tweetId}: ${queueError.message}`);
        continue;
      }
    ...
    console.log(`[DEBUG_SEED] Seeded ${seeded.length}/${count} candidates`);
    
    // Step 2: Wait for transaction commit
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Step 3: Trigger scheduler (in-process, uses same pool)
    const { attemptScheduledReply } = await import('./jobs/replySystemV2/tieredScheduler');
    ...
    res.json({
      ...
      scheduler: {
        ...
        candidate_tweet_id: result.candidate_tweet_id,
      },
      decisions: {
        total: decisions?.length || 0,
        allow: allowCount,
        deny: denyCount,
        deny_reason_breakdown: Object.fromEntries(denyReasons),
      },
      newest_decisions: decisions?.slice(0, 5) || [],
    });
  } catch (error: any) {
    console.error(`[DEBUG_SEED] Error:`, error.message);
    res.status(500).json({
      success: false,
      error: error.message,
    });
