...
// 4. Update tsconfig to include emergency file
const tsconfigPath = 'tsconfig.json';
if (fs.existsSync(tsconfigPath)) {
    let tsconfig = JSON.parse(fs.readFileSync(tsconfigPath, 'utf8'));
    if (!tsconfig.include) tsconfig.include = [];
    if (!tsconfig.include.includes('src/emergencyMain.ts')) {
        tsconfig.include.push('src/emergencyMain.ts');
    }
...
<parameter name="command">node emergency_railway_fix.js