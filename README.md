...
**IMPORTANT:** Some tables must be created manually in Supabase SQL Editor.

#### Creating `reply_opportunities` Table

The reply system requires this table. To create it:

1. Go to **Supabase Dashboard** → **SQL Editor**
2. Click **New Query**
3. Copy the SQL from `create_reply_opportunities_table.sql`
4. Click **Run**
...
**Verify table exists:**
```bash
node -e "require('dotenv').config(); const { createClient } = require('@supabase/supabase-js'); const s = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY); s.from('reply_opportunities').select('*').limit(1).then(({error}) => console.log(error ? '❌ Table missing' : '✅ Table exists'));"
```
...
# Force Railway to redeploy the fix NOW
