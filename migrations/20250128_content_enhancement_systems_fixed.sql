-- ===============================================
-- CONTENT ENHANCEMENT SYSTEMS MIGRATION (FIXED)
-- Implements 5 major upgrades for content quality and diversity
-- Fixed version with proper column handling
-- ===============================================

-- Drop existing tables if they exist to start fresh (optional - comment out if you want to preserve data)
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'prompt_rotation_history_template_id_fkey'
    ) THEN
        ALTER TABLE prompt_rotation_history 
        ADD CONSTRAINT prompt_rotation_history_template_id_fkey 
        FOREIGN KEY (template_id) REFERENCES enhanced_prompt_templates(id);
    END IF;
END $$;
...
-- Learning cycles for performance optimization (fixed)
CREATE TABLE IF NOT EXISTS learning_cycles (
    ...
    is_completed BOOLEAN DEFAULT false,  -- Changed from 'completed' to 'is_completed'
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Add the completed column if it doesn't exist (for backward compatibility)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='learning_cycles' AND column_name='completed') THEN
        ALTER TABLE learning_cycles ADD COLUMN completed BOOLEAN DEFAULT false;
        -- Copy data from is_completed if it exists
        IF EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='learning_cycles' AND column_name='is_completed') THEN
            UPDATE learning_cycles SET completed = is_completed;
        END IF;
    END IF;
END $$;
...
-- Add foreign key for learning cycles after table exists
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'learned_performance_patterns_learning_cycle_id_fkey'
    ) THEN
        ALTER TABLE learned_performance_patterns 
        ADD CONSTRAINT learned_performance_patterns_learning_cycle_id_fkey 
        FOREIGN KEY (learning_cycle_id) REFERENCES learning_cycles(id);
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_learning_cycles_is_completed ON learning_cycles (is_completed);
...
-- Add foreign key for trending topics after table exists
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'trending_topic_usage_topic_id_fkey'
    ) THEN
        ALTER TABLE trending_topic_usage 
        ADD CONSTRAINT trending_topic_usage_topic_id_fkey 
        FOREIGN KEY (topic_id) REFERENCES real_trending_topics(id);
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Created tables: used_idea_fingerprints, content_knowledge_base, enhanced_prompt_templates, prompt_rotation_history, tweet_performance_analysis, learning_cycles, learned_performance_patterns, real_trending_topics, trending_topic_usage, trending_fetch_history';
    RAISE NOTICE 'Created functions: check_idea_fingerprint_usage, get_unused_knowledge_ideas, get_optimal_prompt_template, get_trending_topics_for_content';
    RAISE NOTICE 'Loaded sample data: 8 prompt templates, 10 knowledge base entries';
END $$; 