-- ===============================================
-- ENHANCED AUTONOMOUS LEARNING SYSTEM MIGRATION (FIXED)
...
CREATE TABLE IF NOT EXISTS enhanced_timing_stats (
    ...
    -- Bayesian statistics (reduced precision to avoid overflow)
    alpha_engagement DECIMAL(6,2) DEFAULT 1.0, -- Prior successes + observed successes
    beta_engagement DECIMAL(6,2) DEFAULT 1.0,  -- Prior failures + observed failures
...
CREATE TABLE IF NOT EXISTS contextual_bandit_arms (
    ...
    avg_reward DECIMAL(6,4) DEFAULT 0,
    confidence_bound DECIMAL(6,4) DEFAULT 0,
    
    -- Thompson sampling parameters (reduced precision)
    alpha_param DECIMAL(6,2) DEFAULT 1.0,
    beta_param DECIMAL(6,2) DEFAULT 1.0,
...
CREATE TABLE IF NOT EXISTS contextual_bandit_history (
    ...
    predicted_reward DECIMAL(6,4),
    actual_reward DECIMAL(6,4),
...
CREATE TABLE IF NOT EXISTS budget_optimization_log (
    ...
    -- ROI calculation
    expected_reward DECIMAL(6,4),
    actual_reward DECIMAL(6,4),
    roi_ratio DECIMAL(6,4), -- actual_reward / cost_usd
...
-- ===============================================
-- STORED PROCEDURES FOR ENHANCED LEARNING (FIXED)
-- ===============================================

-- Update timing statistics with simplified Bayesian inference
...
DECLARE
    ...
    new_alpha DECIMAL(6,2);
    new_beta DECIMAL(6,2);
    success_threshold INTEGER := 10; -- Engagement > 10 considered success
    ci_lower DECIMAL(6,4);
    ci_upper DECIMAL(6,4);
    total_ab DECIMAL(6,2);
...
BEGIN
    ...
    IF FOUND THEN
        ...
        -- Calculate simplified confidence intervals
        total_ab := new_alpha + new_beta;
        variance := (new_alpha * new_beta) / (total_ab * total_ab * (total_ab + 1));
        
        -- Simple confidence interval approximation
        ci_lower := GREATEST(0, (new_alpha / total_ab) - 1.96 * SQRT(variance));
        ci_upper := LEAST(1, (new_alpha / total_ab) + 1.96 * SQRT(variance));
    ...
    ELSE
        ...
        INSERT INTO enhanced_timing_stats (
            ...
            CASE WHEN p_engagement > success_threshold THEN 2.0 ELSE 1.0 END,
            CASE WHEN p_engagement > success_threshold THEN 1.0 ELSE 2.0 END,
            0.1, 0.9
        );
    END IF;
END;
