...
exports.trackVerificationFailure = trackVerificationFailure;
exports.verifyTruthIntegrity = verifyTruthIntegrity;
 ...
 */
async function fetchDecisions(hoursAgo) {
 ...
 */
function checkFalseSuccess(decisions) {
    const violations = [];
 ...
 */
function checkSalvageable(decisions) {
    const violations = [];
    for (const d of decisions) {
        const hasTweetId = d.tweet_id && d.tweet_id.trim() !== '';
        const hasThreadIds = d.thread_tweet_ids && d.thread_tweet_ids.trim() !== '' && d.thread_tweet_ids !== '[]';
 ...
 */
function checkIdempotency(decisions) {
    const violations = [];
    for (const d of decisions) {
        if (d.tweet_id && d.thread_tweet_ids && d.thread_tweet_ids !== '[]') {
            try {
                const threadIds = JSON.parse(d.thread_tweet_ids);
 ...
 */
async function verifyOnX(decisions, sampleSize) {
    const violations = [];
 ...
 * Generate report
 */
function generateReport(decisions, falseSuccess, salvageable, idempotency, suspect) {
 ...
 */
async function verifyTruthIntegrity() {
    try {
 ...
 */
async function trackVerificationFailure() {
    try {
        const supabase = getSupabase();
...
//# sourceMappingURL=verifyTruthIntegrity.js.map