...
const SocialContentOperator_1 = require("../src/content/SocialContentOperator");
const brandProfiles_1 = require("../src/content/brandProfiles");
...
async function main() {
    ...
    const brandProfile = (0, brandProfiles_1.getBrandProfile)(args.brand || 'health');
    const operator = new SocialContentOperator_1.SocialContentOperator(brandProfile);
    try {
        if (args.quick) {
...
async function runQuickGeneration(operator, args) {
    ...
    const contentPack = await operator.generateContentPack(seeds, [], // No recent posts
    [], // No target posts
    [] // No mentions
    );
...
async function runAnalysis(operator, args) {
    if (!args.postsFile) {
    ...
    const contentPack = await operator.generateContentPack([], // No new seeds
    postsData, [], // No target posts
    [] // No mentions
    );
...
async function runInteractiveMode(operator, args) {
    ...
    const demoSeeds = [
        { topic: 'sleep optimization', priority: 'high' },
        { topic: 'energy management', priority: 'medium' },
        { topic: 'focus techniques', priority: 'medium' }
    ];
...
async function runStandardGeneration(operator, args) {
...
//# sourceMappingURL=content-operator.js.map