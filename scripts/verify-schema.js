...
const supabaseClients_1 = require("../src/lib/supabaseClients");
    ...
    try {
        ...
        for (const table of uniqueTables) {
            ...
            try {
                ...
                const { data, error } = await supabaseClients_1.admin
                    .from(table)
                    .select('*')
                    .limit(1);
                if (error) {
                ...
                const { error: columnError } = await supabaseClients_1.admin
                    .from(table)
        ...
        try {
            const { error: writeError } = await supabaseClients_1.admin
                .from('learning_posts')
                .upsert([{
            ...
            if (writeError) {
                ...
                await supabaseClients_1.admin.from('learning_posts').delete().eq('tweet_id', testTweetId);
            }
        }
        catch (err) {
        ...
        try {
            ...
            if (writeError) {
                ...
                // Clean up test record
                await supabaseClients_1.admin.from('tweet_metrics').delete().eq('tweet_id', testTweetId);
            }
        }
        catch (err) {
        ...
        try {
            const { error: anonError } = await supabaseClients_1.anon
                .from('learning_posts')
                .insert([{
        ...
        try {
            const { error: notifyError } = await supabaseClients_1.admin.rpc('pg_notify', {
                channel: 'pgrst',
                payload: 'reload schema'
            });
            if (notifyError) {
                // Try alternative method via direct SQL
                const { error: sqlError } = await supabaseClients_1.admin
