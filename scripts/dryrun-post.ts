...
import { ThreadComposer } from '../src/content/threadComposer';
...
interface DryRunOptions {
  ...
  topic?: string;
  targetTweet?: string; // For reply testing
  templateType?: 'mini' | 'deep' | 'checklist';
}
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      case '--type':
        options.type = args[++i] as 'single' | 'thread' | 'reply';
        break;
      case '--topic':
      ...
      case '--target-tweet':
        ...
        break;
      case '--template':
        options.templateType = args[++i] as 'mini' | 'deep' | 'checklist';
        break;
      case '--help':
        showHelp();
        process.exit(0);
  ...
  if (options.templateType) console.log(`📋 Template: ${options.templateType}`);
  if (options.targetTweet) console.log(`🎯 Target Tweet: ${options.targetTweet}`);
  console.log('');
...
async function runThreadDryRun(options: DryRunOptions) {
...
async function runSingleDryRun(options: DryRunOptions) {
...
async function runReplyDryRun(options: DryRunOptions) {
