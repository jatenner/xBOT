    ...
    try {
        ...
        for (const file of expensiveFiles) {
            try {
                ...
                if (!content.includes('EMERGENCY_COST_CHECK') && content.includes('openai.chat.completions.create')) {
                    const newContent = content.replace(/const response = await this\.openai\.chat\.completions\.create/g, `// EMERGENCY_COST_CHECK: Check daily budget before expensive API calls
        ...
        const response = await this.openai.chat.completions.create`);
                    if (newContent !== content) {
                        (0, fs_1.writeFileSync)(file, newContent, 'utf-8');
...
async function analyzeCostPatterns() {
    ...
    try {
        const result = await Promise.resolve().then(() => __importStar(require('child_process'))).then(cp => cp.execSync('grep -r "gpt-4o\\|gpt-4" src/ --include="*.ts" | wc -l', { encoding: 'utf-8' }));
    ...
    try {
        const result = await Promise.resolve().then(() => __importStar(require('child_process'))).then(cp => cp.execSync('grep -r "max_tokens.*[0-9]\\{4,\\}" src/ --include="*.ts" | wc -l', { encoding: 'utf-8' }));
...
//# sourceMappingURL=emergency-cost-fix.js.map