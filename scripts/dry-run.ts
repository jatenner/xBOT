...
import AuthoritativeHealthEngine from '../src/content/authoritativeEngine';
import ContentPolicy from '../src/content/contentPolicy';
import BulletproofComposer from '../src/posting/bulletproofComposer';
...
async function runDryRun(): Promise<void> {
  ...
  const engine = AuthoritativeHealthEngine.getInstance();
  const policy = ContentPolicy.getInstance();
  ...
  try {
    ...
    if (dbHealth.connected) {
      ...
      // Test read operation
      const recentPosts = await supabaseClient.safeSelect('posts', { limit: 1 });
    ...
    const cacheHealth = await redisCache.health();
    ...
    const recentPostsForLearning = await supabaseClient.safeSelect('posts', {
      order: { column: 'created_at', ascending: false },
      limit: 10
    });

    if (recentPostsForLearning.success && recentPostsForLearning.data) {
      const realDataCount = recentPostsForLearning.data.filter(post => 
        !post.tweet_id?.includes('mock') && 
        !post.tweet_id?.includes('test') && 
        !post.tweet_id?.includes('dry_run')
      ).length;
