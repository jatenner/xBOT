...
const EnhancedContentGenerator_1 = require("../src/content/EnhancedContentGenerator");
const supabase_js_1 = require("@supabase/supabase-js");
const playwright_1 = require("playwright");
const ioredis_1 = __importDefault(require("ioredis"));
class EnhancedContentOperator {
    constructor() {
        this.generator = new EnhancedContentGenerator_1.EnhancedContentGenerator();
     ...
     */
    async runPostingPipeline(dryRun = true) {
     ...
     */
    async runRepliesPipeline(dryRun = true) {
     ...
     */
    async postContent(candidate, dryRun) {
        if (dryRun) {
        ...
        try {
            ...
            if (process.env.TWITTER_SESSION_B64) {
                const sessionState = JSON.parse(Buffer.from(process.env.TWITTER_SESSION_B64, 'base64').toString());
     ...
     */
    async logToSupabase(generationResult, postingResult) {
        try {
     ...
     * Generate contextual reply
     */
    async generateContextualReply(target) {
        const prompt = `
...
Reply:`;
     ...
     */
    analyzeEngagementPatterns(posts) {
        ...
        return patterns.map(pattern => {
            ...
            const totalEngagement = patternPosts.reduce((sum, p) => sum + ((p.likes || 0) + (p.replies || 0) + (p.reposts || 0)), 0);
     ...
     */
    extractStructureInsights(pattern, highPerformers) {
        ...
        let insight = insights[pattern] || 'Pattern shows promise';
     ...
     */
    async updateLearningCache(patterns) {
        try {
            const learningData = {
                patterns,
    ...
    recommendNextTopic(patterns) {
        ...
        return topicsByPattern[topPattern?.type] || 'metabolic health';
    }
}
// CLI Interface
async function main() {
...
exports.default = EnhancedContentOperator;
//# sourceMappingURL=enhanced-content-operator.js.map