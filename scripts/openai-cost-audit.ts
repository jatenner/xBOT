...
interface APICallSite {
  ...
  bypassesCostControl: boolean;
  riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
}

interface CostAuditResult {
  totalCallSites: number;
  bypassingCostControl: number;
...
async function auditOpenAIUsage(): Promise<CostAuditResult> {
...
function analyzeAPICall(context: string, file: string): {
  model: string;
  maxTokens: number;
  estimatedCostPerCall: number;
  bypassesCostControl: boolean;
  ...
  const pricing = modelPricing[model as keyof typeof modelPricing] || modelPricing['gpt-4o-mini'];
...
function generateRecommendations(callSites: APICallSite[]): string[] {
