...
CREATE TABLE IF NOT EXISTS style_performance (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  style_type VARCHAR(50) NOT NULL UNIQUE, -- 'breaking_news', 'educational', 'viral_take', 'data_story'
  avg_engagement DECIMAL(5,2) DEFAULT 0,
  total_posts INTEGER DEFAULT 0,
  success_rate DECIMAL(3,2) DEFAULT 0,
...
-- Function to update content theme performance (safe version)
CREATE OR REPLACE FUNCTION update_theme_performance(
  p_theme_name VARCHAR(255),
  p_engagement_score DECIMAL(5,2),
  p_is_success BOOLEAN DEFAULT NULL
)
RETURNS VOID AS $$
BEGIN
  -- Update or insert content theme performance
  INSERT INTO content_themes (theme_name, keywords, total_posts, avg_engagement, success_rate, last_used, updated_at)
  VALUES (
    p_theme_name, 
    ARRAY[p_theme_name], 
    1, 
  ...
  ON CONFLICT (theme_name) DO UPDATE SET
    total_posts = content_themes.total_posts + 1,
    avg_engagement = (content_themes.avg_engagement * content_themes.total_posts + p_engagement_score) / (content_themes.total_posts + 1),
    success_rate = CASE 
      WHEN p_is_success IS NOT NULL THEN
        (content_themes.success_rate * content_themes.total_posts + CASE WHEN p_is_success THEN 1.0 ELSE 0.0 END) / (content_themes.total_posts + 1)
      ELSE content_themes.success_rate
    END,
    last_used = NOW(),
    updated_at = NOW();
END;
...
SELECT 'AI LEARNING BRAIN DATABASE CREATED SUCCESSFULLY - YOUR AGENTS CAN NOW LEARN AND REMEMBER!' as status; 