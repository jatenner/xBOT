...
SELECT 
    ...
    details->>'learning_engine' as engine_version,
    (details->>'tweets_processed')::INTEGER as tweets_processed,
    (details->>'average_engagement')::DECIMAL as avg_engagement,
    (details->>'total_rewards')::INTEGER as total_rewards,
    (details->>'bandit_updates')::INTEGER as bandit_updates,
    details->'top_topics' as top_topics,
    details->'best_hours' as best_hours,
    created_at as logged_at
FROM audit_log 
WHERE event = 'LEARNING_CYCLE_COMPLETE'
...
CREATE INDEX IF NOT EXISTS idx_audit_log_learning 
    ON audit_log(created_at DESC, event) 
    WHERE event = 'LEARNING_CYCLE_COMPLETE';
...
VALUES (
    'MIGRATION_COMPLETE',
    jsonb_build_object(
        'component', 'learning_v2_analytics',
        'migration', '20250812_0002_learning_v2_analytics',
        'views_created', 6,
        'indexes_created', 6,
