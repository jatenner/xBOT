...
import { testEnhancedGrowthEngine } from '../src/agents/growthEngineEnhanced';
import { TwitterSessionRecovery, quickSessionCheck } from '../src/utils/twitterSessionRecovery';
...
async function main() {
  ...
  try {
    const sessionCheck = await quickSessionCheck();
  ...
  try {
    const growthResult = await testEnhancedGrowthEngine();
