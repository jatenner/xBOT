-- =====================================================================================
-- 🚀 COMPREHENSIVE DATABASE SCHEMA FIX - ENTERPRISE GRADE
-- =====================================================================================
-- 
-- PURPOSE: Complete database schema rebuild with:
...
CREATE TABLE tweets (
    ...
    content_type VARCHAR(50) DEFAULT 'single' NOT NULL, -- 'single', 'thread', 'reply', 'quote', 'retweet'
    content_category VARCHAR(100) NOT NULL, -- 'health', 'nutrition', 'fitness', 'wellness', etc.
    content_theme VARCHAR(100), -- 'hydration', 'sleep', 'exercise', etc.
    content_tone VARCHAR(50), -- 'educational', 'humorous', 'motivational', etc.
    ...
    media_type VARCHAR(20), -- 'photo', 'gif', 'video', 'poll', null
    ...
    uniqueness_score DECIMAL(3,2), -- 0.00 to 1.00 (vs recent content)
    ...
    ip_address INET,
    source_inspiration TEXT, -- What inspired this tweet
    editorial_notes TEXT
);

-- Comprehensive indexes for tweets table
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_tweet_id ON tweets(tweet_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_posted_at ON tweets(posted_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_status ON tweets(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_platform ON tweets(platform);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_content_type ON tweets(content_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_content_category ON tweets(content_category);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_engagement_score ON tweets(engagement_score DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_viral_coefficient ON tweets(viral_coefficient DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_ai_model ON tweets(ai_model_used);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_hashtags ON tweets USING GIN(hashtags);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_mentions ON tweets USING GIN(mentions);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_created_at ON tweets(created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tweets_deleted_at ON tweets(deleted_at) WHERE deleted_at IS NOT NULL;
...
CREATE TABLE bot_config (
    id SERIAL PRIMARY KEY,
    key VARCHAR(100) NOT NULL, -- Standardized to 'key' not 'config_key'
    value JSONB NOT NULL, -- Standardized to 'value' not 'config_value'
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bot_config_type ON bot_config(config_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bot_config_active ON bot_config(is_active);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bot_config_env ON bot_config(environment);
...
CREATE TABLE IF NOT EXISTS twitter_auth_sessions (
    ...
    auth_status VARCHAR(20) DEFAULT 'pending' NOT NULL, -- 'pending', 'authenticated', 'expired', 'failed'
    auth_method VARCHAR(20) DEFAULT 'oauth1' NOT NULL, -- 'oauth1', 'oauth2', 'bearer'
    
    -- Token information (encrypted)
    access_token_hash VARCHAR(64), -- SHA-256 hash for security
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_auth_sessions_status ON twitter_auth_sessions(auth_status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_auth_sessions_user_id ON twitter_auth_sessions(twitter_user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_auth_sessions_expires ON twitter_auth_sessions(expires_at);
...
CREATE TABLE IF NOT EXISTS content_analytics (
    ...
    -- Advanced metrics
    engagement_rate DECIMAL(5,4) DEFAULT 0.0000 NOT NULL,
    viral_velocity DECIMAL(5,4) DEFAULT 0.0000 NOT NULL, -- Growth rate
    reach_efficiency DECIMAL(5,4) DEFAULT 0.0000 NOT NULL, -- Engagement per impression
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_analytics_tweet_id ON content_analytics(tweet_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_analytics_recorded_at ON content_analytics(recorded_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_analytics_hours_since ON content_analytics(hours_since_posted);
...
CREATE TABLE IF NOT EXISTS ai_learning_data (
    ...
    event_type VARCHAR(50) NOT NULL, -- 'content_performance', 'timing_optimization', 'engagement_prediction'
    learning_category VARCHAR(50) NOT NULL, -- 'content_quality', 'viral_prediction', 'optimal_timing'
    
    -- Input data
    input_features JSONB NOT NULL, -- Features used for prediction/decision
    expected_outcome JSONB, -- What we expected to happen
    actual_outcome JSONB NOT NULL, -- What actually happened
    
    -- Learning metrics
    accuracy_score DECIMAL(5,4), -- How accurate was our prediction
    confidence_score DECIMAL(5,4), -- How confident were we
    improvement_potential DECIMAL(5,4), -- How much we can improve
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_learning_event_type ON ai_learning_data(event_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_learning_category ON ai_learning_data(learning_category);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_learning_accuracy ON ai_learning_data(accuracy_score DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_learning_created_at ON ai_learning_data(created_at DESC);
...
CREATE TABLE IF NOT EXISTS system_monitoring (
    ...
    component_name VARCHAR(100) NOT NULL, -- 'twitter_api', 'database', 'ai_service', 'posting_engine'
    status VARCHAR(20) NOT NULL DEFAULT 'unknown', -- 'healthy', 'degraded', 'critical', 'down'
    ...
    -- Health scores
    availability_score DECIMAL(5,4) DEFAULT 1.0000 NOT NULL, -- 0.0000-1.0000
    ...
    -- Additional metadata
    version_info JSONB,
    configuration_hash VARCHAR(64), -- Track config changes
    environment VARCHAR(20) DEFAULT 'production' NOT NULL
);

-- Indexes for monitoring
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_monitoring_timestamp ON system_monitoring(check_timestamp DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_monitoring_component ON system_monitoring(component_name);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_monitoring_status ON system_monitoring(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_monitoring_health_score ON system_monitoring(overall_health_score DESC);
...
CREATE TABLE IF NOT EXISTS content_queue (
    ...
    content_priority INTEGER DEFAULT 5 NOT NULL, -- 1-10 priority scale
    
    -- Scheduling
    scheduled_for TIMESTAMP WITH TIME ZONE NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    status VARCHAR(20) DEFAULT 'scheduled' NOT NULL, -- 'scheduled', 'posting', 'posted', 'failed', 'cancelled'
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_queue_scheduled_for ON content_queue(scheduled_for);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_queue_status ON content_queue(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_queue_priority ON content_queue(content_priority DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_queue_category ON content_queue(content_category);
...
-- =====================================================================================
-- 12. DATA VALIDATION AND CLEANUP
-- =====================================================================================

-- Clean up any orphaned or invalid data
DELETE FROM content_analytics WHERE tweet_id NOT IN (SELECT id FROM tweets);
DELETE FROM ai_learning_data WHERE reference_tweet_id IS NOT NULL AND reference_tweet_id NOT IN (SELECT id FROM tweets);

-- Update any tweets with missing engagement calculations
UPDATE tweets 
SET engagement_score = (likes * 1) + (retweets * 3) + (replies * 2) + (quotes * 4) + (bookmarks * 2)
WHERE engagement_score = 0 AND (likes > 0 OR retweets > 0 OR replies > 0 OR quotes > 0 OR bookmarks > 0);
...
INSERT INTO system_monitoring (
    ...
    'healthy',
    1.0000,
    '{"schema_version": "2.0.0", "migration": "comprehensive_fix", "timestamp": "' || NOW()::text || '"}'
);
...
BEGIN
    ...
    RAISE NOTICE '  ✅ tweets - Enhanced with full analytics and AI metadata';
    RAISE NOTICE '  ✅ bot_config - Unified configuration system with Twitter auth';
    RAISE NOTICE '  ✅ twitter_auth_sessions - Secure authentication management';
    RAISE NOTICE '  ✅ content_analytics - Time-series performance tracking';
    RAISE NOTICE '  ✅ ai_learning_data - Machine learning optimization';
    RAISE NOTICE '  ✅ system_monitoring - Health and performance tracking';
    RAISE NOTICE '  ✅ content_queue - Advanced scheduling system';
    RAISE NOTICE '';
    RAISE NOTICE 'PERFORMANCE FEATURES:';
    RAISE NOTICE '  ✅ 25+ optimized indexes for fast queries';
    RAISE NOTICE '  ✅ Automated triggers for engagement calculations';
    RAISE NOTICE '  ✅ Performance dashboard views';
    RAISE NOTICE '  ✅ Row Level Security (RLS) policies';
    RAISE NOTICE '';
    RAISE NOTICE 'NEXT STEPS:';
    RAISE NOTICE '  1. Update environment variables with Twitter API credentials';
    RAISE NOTICE '  2. Test Twitter authentication with updated schema';
    RAISE NOTICE '  3. Deploy bot with new unified configuration system';
    RAISE NOTICE '  4. Monitor system health via new monitoring tables';
    RAISE NOTICE '';
    RAISE NOTICE '🎯 Your bot is now ready for enterprise-grade operation!';
    RAISE NOTICE '🚀 ====================================================================';
    RAISE NOTICE '';
END $$;